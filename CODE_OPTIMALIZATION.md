...
- The current Linux workflow builds both a `.deb` artifact and an AppImage portable artifact. AppImage packaging uses Debian 13 distro Qt, Qt's supported deployment script, and pinned `appimagetool`/runtime inputs instead of `linuxdeployqt` or `linuxdeploy-plugin-qt`.
