...
- [x] Phase 6 partial: Linux packaging workflow now builds and smoke-installs the `.deb` artifact inside the Ubuntu 26.04 baseline, names that artifact with an `ubuntu-26.04` suffix, and uses AppImage smoke coverage for Debian 13 and other non-matching distributions.
- [x] Phase 6 partial: Linux packaging workflow no longer executes mutable `linuxdeployqt` `continuous` AppImage downloads.
- [x] Phase 6 partial: Linux packaging now builds an AppImage through Qt's generated Linux deployment script inside a Debian 13 container, uses `scripts/prepare_linux_appimage_appdir.sh` to stage Qt plugin groups plus `ldd`-resolved Qt runtime libraries and wrap AppRun with runtime library/plugin paths, preflights the AppDir launch, records tool provenance in the artifact manifest, and smoke-launches the AppImage on Ubuntu 26.04 and Debian 13.
...
- The current Linux workflow builds both a `.deb` artifact and an AppImage portable artifact. AppImage packaging uses Debian 13 distro Qt, Qt's supported deployment script, explicit Qt runtime library staging, an AppRun path wrapper, and pinned `appimagetool`/runtime inputs instead of `linuxdeployqt` or `linuxdeploy-plugin-qt`.
