...
- [x] Phase 6 partial: Linux packaging now builds an AppImage through Qt's generated Linux deployment script inside a Debian 13 container, uses `scripts/prepare_linux_appimage_appdir.sh` to stage `ldd`-resolved Qt runtime libraries and wrap AppRun with runtime library/plugin paths, preflights the AppDir launch, records tool provenance in the artifact manifest, and smoke-launches the AppImage on Ubuntu 26.04 and Debian 13.
