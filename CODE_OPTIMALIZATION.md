...
- [x] Phase 6 partial: Windows installer workflow now validates exact installer artifact naming against `THERION_STUDIO_PACKAGE_LABEL`, uses `dev-<short_sha>` labels for snapshot installers, and emits a SHA256 manifest via `scripts/verify_windows_installer_artifact.py` before upload.
- [x] Phase 6 partial: Windows installer workflow now runs daily on the default branch in addition to manual dispatch, using Qt 6.8.3 and `Release` builds for scheduled validation.
...
- [x] Phase 6 partial: Linux release artifacts are now naming-verified and accompanied by SHA256 manifest metadata through `scripts/linux-packages/verify_release_artifacts.py`.
...
- [x] Phase 6 partial: Linux packaging workflow now builds `amd64`/`arm64` `.deb` artifacts through `scripts/linux-packages/build_deb_package.sh`, builds `x86_64`/`aarch64` AppImages through `scripts/linux-packages/build_appimage_package.sh`, consolidates Linux package build/smoke helpers under `scripts/linux-packages/`, provides Docker wrappers and a full local build+smoke wrapper for local reproduction, names `.deb` artifacts with an `ubuntu-26.04` suffix, and uses Docker-backed smoke scripts for Ubuntu 26.04 `.deb` plus Debian 13/Ubuntu 26.04 AppImage coverage.
...
- The current Linux workflow builds `amd64`/`arm64` `.deb` artifacts and `x86_64`/`aarch64` AppImage portable artifacts. AppImage packaging uses Debian 13 distro Qt, Qt's supported deployment script, explicit Qt runtime library staging, an AppRun path wrapper, and pinned architecture-specific `appimagetool`/runtime inputs instead of `linuxdeployqt` or `linuxdeploy-plugin-qt`.
