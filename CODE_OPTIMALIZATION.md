...
- [x] Phase 6 partial: Linux packaging now builds an AppImage through Qt's generated Linux deployment script using workflow-provisioned Qt 6.8.3 plus pinned `appimagetool`/runtime inputs, records tool provenance in the artifact manifest, and smoke-launches the AppImage on Ubuntu 26.04 and Debian 13.
...
- The current Linux workflow builds both a `.deb` artifact and an AppImage portable artifact. AppImage packaging uses Qt's supported deployment script with workflow-provisioned Qt 6.8.3 and pinned `appimagetool`/runtime inputs instead of `linuxdeployqt` or `linuxdeploy-plugin-qt`.
