...
- Linux AppImage production workflows that use Qt's CMake deployment API shall build the portable artifact with a Qt version that supports Linux deployment through that API, and shall record the Qt package source, version, and selected package set in release provenance
...
- Linux `.deb` packaging should remain compatible with the Qt 6 packages shipped by supported Ubuntu distributions where practical; Linux CI shall include distribution-package Qt builds for supported Ubuntu baselines, including Ubuntu 24.04 and Ubuntu 26.04, and release-artifact `.deb` smoke installation on Ubuntu 26.04. Debian compatibility shall be covered by the AppImage channel unless a separate Debian-built `.deb` artifact is introduced.
