...
- The `Selection` inspector shall group controls in this order: selected-object section, `Geometry`, `Point / Vertex`, and `Actions`, so object identity edits, object-level geometry state controls, point/vertex actions, and object actions remain visually distinct.
