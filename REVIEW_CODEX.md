...
- `TherionSourceText` is now the shared line/newline helper for the migrated call sites, and the first parser snapshot API with line/token ranges exists; most projection migration and broader transaction integration still need migration.
...
- `TherionDocumentEditor::rewritePointCoordinates()` now uses absolute coordinate token ranges from the lossless source snapshot instead of split/join line reconstruction.
- `TherionDocumentEditor::rewriteMapObjectQuickFields()` now replaces only the affected physical source line from the lossless source snapshot instead of split/join line reconstruction.
- `TherionDocumentEditor::rewriteMapObjectTextOption()` and `rewriteMapObjectValueOption()` now replace only the affected physical source line from the lossless source snapshot instead of split/join line reconstruction.
...
- `TherionDocumentEditor::rewriteLinePointOrientation()` and `rewriteLinePointLeftSize()` now route their shared standalone line-point option insert/update/remove helper through lossless source-line ranges instead of split/join full-document reconstruction.
- `TherionDocumentEditor::appendDraftGeometryEdits()`, `appendDraftLineGeometryEdits()`, and `appendDraftAreaGeometryEdits()` now plan generated draft geometry insertion through lossless physical source-line ranges instead of split/join full-document reconstruction.
