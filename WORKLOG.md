...
- Linux AppImage packaging depends on workflow-provisioned Qt 6.8.3, Qt host-tool `bin`/`lib` runtime paths, Qt's Linux deploy script, and pinned `appimagetool`/runtime inputs; verify the full path in GitHub Actions on Ubuntu 26.04 and Debian 13 before treating it as stable.
