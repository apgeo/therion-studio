...
- P3: Keep Linux release artifact verification stable across `.deb` + `AppImage` workflow updates.
...
- Status: in progress.
- Remaining work: harden release process and artifacts after the current Windows-installer + Homebrew + Linux (`.deb`/`AppImage`) flow, including signing/notarization expectations.
