...
- Status: in progress.
- Remaining work: harden release process and artifacts after the current Windows-installer + Homebrew + Linux `.deb`/AppImage flow, validate Docker-reproducible Linux build/smoke scripts across Ubuntu 26.04 and Debian 13, consider APT repository publishing, and keep signing/notarization expectations explicit.
...
- Architecture refactor roadmap: `CODE_OPTIMALIZATION.md` is the staged plan; Phase 1 dependency-injection seams are complete, Phase 2 is complete with extracted main-window session/project/runner workflows, Phase 3 is complete with `MainWindowDocumentController` save/close orchestration, `MainWindowDocumentOpenController` open/reuse planning, `MainWindowDocumentTabOpenController` tab-activation/new-open orchestration, `TextEditorDocumentPersistenceStateService`, `TextEditorDocumentWorkflowController`, `TextEditorDocumentPreconditionsService`, `TextEditorDocumentIoService`, `TextEditorTabInteractionController`, and focused TextEditorTab delegate translation units (`TextEditorTabDocumentModeDelegates.cpp`, `TextEditorTabBootstrap.cpp`, `TextEditorTabStatusCursorDelegates.cpp`, `TextEditorTabSourceRewriteDelegates.cpp`, `TextEditorTabRawFindDelegates.cpp`, `TextEditorTabBlockDetailsDelegates.cpp`), Phase 4 is complete with map-editor shell decomposition across scene refresh, selection/inspector workflows, source-edit workflows, event routing, line-extension planning, workspace/document/status delegates, detached-pane lifecycle/window shell, inspector-panel UI delegates, inspector/object-details controller delegates, and grouped map-editor state surfaces, Phase 5 is complete with centralized startup/bootstrap entrypoints plus shared appearance/theming policies (`LucideIconFactory`, `ApplicationStylePolicy`, and `MapEditorSceneThemePolicy`), and Phase 6 is in progress with CI install-layout smoke checks, Windows artifact naming/manifest verification with `dev-<short_sha>` snapshot labels, Linux `amd64`/`arm64` `.deb` and `x86_64`/`aarch64` AppImage packaging scripts consolidated under `scripts/linux-packages/` with Docker wrappers plus a full local build+smoke wrapper, Ubuntu 26.04 `.deb` smoke checks, Debian 13 plus Ubuntu 26.04 AppImage smoke checks, and deferred signing/notarization scope.
