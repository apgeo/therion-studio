...
- P2: Stabilize the Qt-deploy/AppImage CI path across Ubuntu 26.04 and Debian 13, then consider adding GitHub Pages APT repository publishing for the `.deb` channel.
...
- Linux AppImage packaging depends on Debian 13 distro Qt, Qt's Linux deploy script, pinned `appimagetool`/runtime inputs, and recursive downloaded-artifact discovery in smoke jobs; verify the full path in GitHub Actions on Ubuntu 26.04 and Debian 13 before treating it as stable.
...
- Status: in progress.
- Remaining work: harden release process and artifacts after the current Windows-installer + Homebrew + Linux `.deb`/AppImage flow, validate AppImage CI across Ubuntu 26.04 and Debian 13, consider APT repository publishing, and keep signing/notarization expectations explicit.
