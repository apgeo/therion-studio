...
- Architecture refactor roadmap: `CODE_OPTIMALIZATION.md` is the staged plan; Phase 1 dependency-injection seams are complete after removing legacy real-adapter constructor shortcuts, and Phase 2 is now active with the first `MainWindow` session-document workflow extracted into `MainWindowSessionDocumentService`.
- Track A: continue Phase 2 with focused `MainWindow` extraction, extending the new session-document service slice toward full document/session orchestration before broader UI shell cleanup.
