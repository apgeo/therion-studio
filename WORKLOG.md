...
- Architecture refactor roadmap: `CODE_OPTIMALIZATION.md` is the staged plan; Phase 1 dependency-injection seams are complete after removing legacy real-adapter constructor shortcuts, and Phase 2 is now active with extracted `MainWindowSessionDocumentService`, `MainWindowSessionProjectService`, `MainWindowSessionStateService`, and `MainWindowStructureNameOverridesService` slices for session restore/persist decisions, including active-document persistence precedence.
- Track A: continue Phase 2 with focused `MainWindow` extraction, extending current session-document, project-restore, session-state persistence, and structure-override serialization service slices toward full document/session orchestration before broader UI shell cleanup.
