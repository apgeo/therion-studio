...
- P1 - Architecture roadmap: Phase 2 is complete (session/project/runner workflow extraction from MainWindow); Phase 3 is complete (`TextEditorTab` coupling reduced via document/mode/bootstrap/status/cursor/source-rewrite/raw-find/block-details/context-help/encoding delegate splits). Phase 4 started with scene-refresh/appearance lifecycle extraction into `MapEditorTabSceneRefreshLifecycle.cpp` and selection/inspector insert-workflow extraction into `MapEditorTabSelectionInspectorWorkflow.cpp`; continue with source-edit orchestration splits.
