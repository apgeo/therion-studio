...
- P2: Select a maintained portable Linux production artifact path without mutable `continuous` AppImage tooling or unmaintained Qt deployment plugins.
...
- Linux production release packaging is blocked until a maintained broadly portable artifact path is selected and verified; the current workflow is `.deb` preview/tester coverage only.
...
- Status: in progress.
- Remaining work: harden release process and artifacts after the current Windows-installer + Homebrew + Linux `.deb` preview flow, select the production portable Linux artifact path, and keep signing/notarization expectations explicit.
