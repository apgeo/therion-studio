...
- P1 - Architecture roadmap: Phase 2 is complete (session/project/runner workflow extraction from MainWindow); Phase 3 started with `MainWindowDocumentController` save/close extraction, `MainWindowDocumentOpenController` open/reuse planning extraction, `MainWindowDocumentTabOpenController` tab activation/new-open orchestration extraction, and `TextEditorDocumentPersistenceStateService` + `TextEditorDocumentWorkflowController` + `TextEditorDocumentPreconditionsService` + `TextEditorDocumentIoService` extraction from `TextEditorDocumentController`, plus `TextEditorTabInteractionController` extraction for text/mode interaction orchestration, `TextEditorTabDocumentModeDelegates.cpp` split for mode/document delegate wiring, `TextEditorTabBootstrap.cpp` split for tab bootstrap/build flow with isolated `buildDocumentContext()` assembly, `TextEditorTabStatusCursorDelegates.cpp` split for status/cursor/document-state delegates, and `TextEditorTabSourceRewriteDelegates.cpp` split for source-rewrite/configure/delete routing; continue with broader TextEditorTab coupling reduction.
