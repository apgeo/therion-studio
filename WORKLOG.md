...
- P1 - Localization hygiene: keep Czech/Slovak catalogs complete as new UI strings are added, maintain `docs/LOCALIZATION.md`, avoid translating canonical Therion command syntax/accepted values/examples, keep Therion catalog generation responsible for canonical argument/option signature-vs-description separation, and reduce remaining non-`QObject` translation-helper extraction warnings when touching those controllers.
- P1 - Project index hardening: keep the current lightweight structure sidebar index stable with reusable object/reference data, stable source-backed object IDs, enum-backed object kinds, canonical Therion object-kind labels in the Structure graph, Therion-compatible `centreline`/`centerline` alias handling, Therion-compatible relative namespace resolution for map/scrap references, project-root-scoped Therion config/`source` file graph traversal with resolved root-config/root-file metadata, `thconfig.*` config-name support, target-config disambiguation and structure-pane messaging/action routing for roots with multiple named config files, map-to-map and map-to-scrap reference resolution including Therion offset map item syntax plus unresolved/ambiguous/mixed-content diagnostics surfaced as navigable warning rows outside UI widgets, non-hierarchical handling for `preview above` / `preview below`, structure-sidebar snapshot application guarded by a structural signature to avoid refresh churn while typing, and expand/collapse state preservation across same-project structure refreshes.
- P2 - Structure/export semantics exploration: consider lightweight indexing for map `survey` references, `map -survey <id>` associated-survey metadata, and `select`/`unselect` export overlays without treating the Structure tree as full compiler/export selection emulation.
- P2 - Structure graph view exploration: consider adding a scoped graph mode alongside the current Structure tree, not as a replacement, to visualize Therion relationships such as map/scrap references, `preview`, `revise`, `join`, `equate`, diagnostics, and optional station-network edges with filters and selection-scoped expansion.
...
- P1 - Background layer model/loading cleanup: keep XTherion metadata loading compatible with legacy raster forms such as unbraced `xth_me_image_insert` y/base coordinates, keep gamma edits from rewriting X/Y placement tokens or changing selected layer/scene placement, extract `MapEditorBackgroundLayerModel`/loader responsibilities before adding more background-layer features, move large raster decode/gamma/scaling work behind cancellable async jobs, and add a bounded `.xvi` cache policy or clear-on-project-close hook if large `.xvi` projects become common.
...
- P1 - Windows installer Qt runtime layout: install `TherionStudio.exe` under `bin/` and explicitly stage the Qt Windows platform plugin at `bin/platforms/qwindows.dll` so deployed Qt DLLs and plugin directories stay relative to the executable.
...
- Later ProjectIndex hardening: add compiler-confirmed snapshot comparison by capturing the actual Therion run config/working directory/arguments and comparing it with the lightweight structure index root metadata when lightweight mode is no longer sufficient.
