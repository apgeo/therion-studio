...
- Architecture refactor roadmap: `CODE_OPTIMALIZATION.md` is the staged plan; Phase 1 dependency-injection seams are complete after removing legacy real-adapter constructor shortcuts, and Phase 2 is now active with extracted `MainWindowSessionDocumentService`, `MainWindowSessionDocumentController`, `MainWindowSessionProjectService`, `MainWindowSessionStateService`, `MainWindowSessionRestoreOrchestrationService`, `MainWindowSessionRestoreUiFlowService`, `MainWindowSessionWindowRestoreService`, `MainWindowSessionController`, `MainWindowStructureNameOverridesService`, `MainWindowProjectLifecycleService`, `MainWindowProjectWorkspaceService`, `MainWindowProjectUiFlowService`, `MainWindowProjectOrchestrationService`, and `MainWindowProjectController` slices for session restore/persist decisions and project open/close workflow transitions, including single-call close-project confirmation coordination.
