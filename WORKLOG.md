...
- Architecture refactor roadmap: use `CODE_OPTIMALIZATION.md` as the current staged plan; Phase 0 is checked off, and Phase 1 continues with dependency-injection seams after `TextEditorDocumentController` filesystem injection, `MainWindow` session-store injection, injectable map-inspector command metadata, injectable map-orientation metadata, explicit `MapEditorTab` metadata context wiring, `MapEditorTab` construction from an explicit `CommandCatalogStore`, `TherionSyntaxHighlighter` command metadata injection, shared `TextEditorTab` command catalog ownership for raw help/completion/highlighting, `MainWindow` command catalog ownership for editor tab construction, injected map object-details subtype refresh metadata, app-startup command catalog composition, removal of static default map inspector/orientation helper APIs, explicit raw syntax/help command metadata requirements, embedded map raw-editor catalog reuse, additional-window catalog reuse, and removal of no-catalog UI shell constructor overloads.
