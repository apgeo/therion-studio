...
- ~~Fixed stale/current-location TH2 Visual `Inspector -> Objects` drop-target feedback: hiding the row drop indicator now clears and repaints its previous geometry, planner coverage now asserts both immediate-next and immediate-previous source no-op boundaries, and the GUI drag smoke test now exercises a visible-adjacent-row no-op where hidden source text sits between the visible rows. Updated the user manual. Verified with `cmake --build build --target MapEditorObjectMovePlannerTest MapEditorDragUndoRedoSmokeTest TherionStudio`, `cmake --build build-strict --target MapEditorObjectMovePlannerTest MapEditorDragUndoRedoSmokeTest TherionStudio`, `ctest --test-dir build --output-on-failure -R "MapEditorObjectMovePlannerTest|MapEditorDragUndoRedoSmokeTest|StructureConstraintsTest"`, `ctest --test-dir build-strict --output-on-failure -R "MapEditorObjectMovePlannerTest|MapEditorDragUndoRedoSmokeTest|StructureConstraintsTest"`, `python3 scripts/check_structure_constraints.py`, and `git diff --check`.~~
- ~~Polished TH2 Visual `Inspector -> Objects` row action affordances: visibility/delete action columns now use pointing-hand hover feedback, while label/grip hover remains open-hand row-drag feedback. Expanded `MapEditorDragUndoRedoSmokeTest` to assert the cursor distinction for drag vs action columns. Updated the user manual. Verified with `cmake --build build --target MapEditorDragUndoRedoSmokeTest TherionStudio`, `cmake --build build-strict --target MapEditorDragUndoRedoSmokeTest TherionStudio`, `ctest --test-dir build --output-on-failure -R "MapEditorDragUndoRedoSmokeTest|StructureConstraintsTest"`, `ctest --test-dir build-strict --output-on-failure -R "MapEditorDragUndoRedoSmokeTest|StructureConstraintsTest"`, `python3 scripts/check_structure_constraints.py`, and `git diff --check`.~~
- ~~Suppressed TH2 Visual `Inspector -> Objects` drop-target lines for current-location no-op moves by checking the existing `MapEditorObjectMovePlanner` during drag hover. No-op targets now use forbidden-cursor/status feedback instead of drawing a placement line. Expanded `MapEditorDragUndoRedoSmokeTest` to exercise the no-op hover before a valid move. Updated the user manual. Verified with `cmake --build build --target MapEditorDragUndoRedoSmokeTest TherionStudio`, `cmake --build build-strict --target MapEditorDragUndoRedoSmokeTest TherionStudio`, `ctest --test-dir build --output-on-failure -R "MapEditorDragUndoRedoSmokeTest|StructureConstraintsTest"`, `ctest --test-dir build-strict --output-on-failure -R "MapEditorDragUndoRedoSmokeTest|StructureConstraintsTest"`, `python3 scripts/check_structure_constraints.py`, and `git diff --check`.~~
...
- ~~TH2 Visual `Inspector -> Objects` row drag/drop UI: movable rows expose grip handles, hover/open-hand and drag/closed-hand cursors, pointing-hand row action cursors, a 2 px drop-target line for real moves only, invalid/no-op forbidden cursor/status feedback, edge auto-scroll, leave cancellation, selection restoration, and clean map undo/redo routing.~~
- Planned focus:
