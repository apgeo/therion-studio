...
- P1 - Architecture roadmap: Phase 2 is complete (session/project/runner workflow extraction from MainWindow); Phase 3 started with `MainWindowDocumentController` save/close extraction, `MainWindowDocumentOpenController` open/reuse planning extraction, `MainWindowDocumentTabOpenController` tab activation/new-open orchestration extraction, and `TextEditorDocumentPersistenceStateService` + `TextEditorDocumentWorkflowController` + `TextEditorDocumentPreconditionsService` extraction from `TextEditorDocumentController`; continue with TextEditorTab/document-workflow coupling reduction.
