...
- P1 - Line decoration style coverage: keep JSON-driven `stroke_visible` and `decorations` support for symbol-only and compound line symbols (`symbols`, `teeth`, `ticks`, `offset_stroke`, `parallel`, `rungs`) aligned with Therion symbol-set coverage and tune bundled line subtype presets.
- P1 - Map insertion inspector workflow: continue validating pending-insert `Selection` inspector defaults, reliable `Esc` cancellation from focused inspector fields and command-toolbar focus, cleared-selection empty-state behavior, screen-space area/line hit testing, full-width style-preview layout, style-preview fidelity including line-decoration spacing/color contrast on the light preview surface, area tile coverage, decoration jitter, and dark-mode contrast, and the two-step scrap insertion UX across point/line/freehand/area workflows.
