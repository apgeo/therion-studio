...
The `Structure` pane shows detected `survey`, `centerline`, `map`, and `scrap` hierarchy from open content. Both Therion spellings, `centreline` and `centerline`, are recognized. Structure graph object-kind labels use canonical Therion source terms instead of translated labels. Maps and scraps referenced inside a `map ... endmap` block are nested under that map when the reference is unambiguous, including map references that use Therion offset syntax such as `child-map.m [10 20 m] above`. References follow Therion namespace rules: unqualified names resolve in the owning map namespace, while `name@child.parent` resolves relative to the owning map namespace. Unresolved, ambiguous, or mixed map/scrap references appear as warning rows under the owning map and open the source reference line when selected. `preview above` / `preview below` relations are shown as map source content, not as ownership hierarchy. The `Structure` pane is a lightweight navigation/indexing aid; the Therion compiler remains authoritative for full validation and export selection behavior. Selecting an item opens/synchronizes to its source location.
