...
Starting `Point`, `Line`, `Freehand`, or `Area` activates `Inspector -> Selection` before the first point or vertex is placed. Set type, subtype, ID, point name, label text, or supported point value there before committing the new object. The editor remembers recent type/subtype choices separately for new points, lines, and areas, uses the latest choice as the next default while the map editor stays open, and shows recent choices as compact quick buttons in `Recent` below the symbol preview. If a scrap or an object inside a scrap was selected when you started the tool, the new object is inserted into that scrap; the pending metadata line shows the target scrap ID. Use `Insert into` to choose a different existing target scrap before committing.
