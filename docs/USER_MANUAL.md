...
The 3D viewer is read-only. Its toolbar provides `Fit`, `Reset`, `Measure`, `Top View`, `Side View`, `Rotate Left`, and `Rotate Right` controls, left-drag orbit navigation, right- or middle-drag pan navigation, mouse-wheel zoom, and rotate buttons that spin the view around the blue/world axis. The inspector panel lets you switch model coloring between survey-based coloring and depth-based coloring, and the centerline uses the same palette. The viewport overlays a cave length and cave depth summary computed from underground centerline shots only, while the measurement tool is a toggle: click the ruler button, then click two stations to inspect their 3D distance, azimuth, and vertical difference; click the ruler button again to leave measurement mode. Hover a station marker to see its full reference and depth in a floating detail card near the cursor, even if station labels or markers are hidden. The 3D canvas uses a black background, draws a red bounding box around the loaded 3D scene extent when bounds are available, and overlays an altitude legend with a compass and a right-side view-angle semicircle beneath it, with the scale bar rendered as a simple line with end ticks beside that control cluster when the scene has valid bounds. The altitude legend includes multiple intermediate elevation labels. The view-angle needle moves through the upper half for above-scene views and through the lower half for below-scene views.
