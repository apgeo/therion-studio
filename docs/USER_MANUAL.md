...
The 3D viewer is read-only. Its toolbar provides `Fit`, `Reset`, `Measure`, `Top View`, `Side View`, `Rotate Left`, and `Rotate Right` controls, left-drag orbit navigation, right- or middle-drag pan navigation, mouse-wheel zoom, and rotate buttons that spin the view around the blue/world axis. The inspector panel also lets you switch mesh coloring between survey-based coloring and depth-based coloring, and the centerline uses the same palette. The measurement tool is a toggle: click the ruler button, then click two stations to inspect their 3D distance, azimuth, and vertical difference; click the ruler button again to leave measurement mode. Hover a station marker to see its full reference and depth in a floating detail card, even if station labels or markers are hidden. The 3D canvas uses a black background, draws a red bounding box around the loaded 3D scene extent when bounds are available, and overlays a compass, a scale bar, and an altitude legend when the scene has valid bounds.
